...
exports.getroadmap = catchAsyncErrors(async (req, res, next) => {
  try {
    ...
    let { txtpath, txtname } = await txtCreater(`${formdata.fullname}-${Date.now()}`, roadmap, formdata.fullname, formdata);
    ...
    console.error('Error generating roadmap:', error); // Print the error to the console
    res.status(500).json({
      success: false,
